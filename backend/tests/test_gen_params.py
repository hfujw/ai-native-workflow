"""生成参数（前端设置）→ orchestrator 会话级覆盖测试（计费已砍，无 budget）。"""
...
def test_params_partial_override():
    ctx = _base_ctx()
    apply_gen_params(ctx, {"searchMax": 2})
    assert ctx["search_max"] == 2
